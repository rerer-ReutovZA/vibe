...
GameDataCapture::GameDataCapture()
    : isRunning_(false)
    , shouldStop_(false)
...
bool GameDataCapture::initialize(const std::string& wsHost, int wsPort) {
    LOG_INFO("Initializing GameDataCapture...");
    ...
    LOG_INFO("GameDataCapture initialized successfully");
    return true;
}

bool GameDataCapture::start() {
    if (isRunning_) {
        LOG_WARNING("GameDataCapture is already running");
        return true;
    }
    ...
    captureThread_ = std::make_unique<std::thread>(&GameDataCapture::captureThreadFunc, this);

    LOG_INFO("GameDataCapture started");
    return true;
}

void GameDataCapture::stop() {
    if (!isRunning_) {
        return;
    }
    ...
    LOG_INFO("GameDataCapture stopped");
}

bool GameDataCapture::isRunning() const {
    return isRunning_;
}

void GameDataCapture::setUpdateInterval(int intervalMs) {
    updateIntervalMs_ = intervalMs;
    LOG_INFO("Update interval set to " + std::to_string(intervalMs) + "ms");
}

GameState GameDataCapture::getCurrentGameState() {
    return lastState_;
}

void GameDataCapture::captureThreadFunc() {
...
bool GameDataCapture::capturePlayerData(GameState& state) {
    if (!memoryReader_) {
        return false;
    }
...
bool GameDataCapture::captureBombData(GameState& state) {
    if (!memoryReader_) {
        return false;
    }
...
bool GameDataCapture::captureGameEvents(GameState& state) {
...
bool GameDataCapture::scanForOffsets() {
